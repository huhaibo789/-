...
public class FileUtilspassword {
    /**
     * 将数据保存到文件
     */
    public static void saveDataToFile(Context context, String data) {
        try {
            ...
            // mode代表的是打开模式
            FileOutputStream fos = context.openFileOutput("test2.txt", Context.MODE_PRIVATE);
    ...
    public static String readDataFromFile(Context context) {
        try {
            FileInputStream fis = context.openFileInput("test2.txt");
